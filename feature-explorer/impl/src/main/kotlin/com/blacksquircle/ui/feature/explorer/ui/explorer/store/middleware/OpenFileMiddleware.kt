...
internal class OpenFileMiddleware @Inject constructor(
    ...
    override fun bind(state: Flow<ExplorerState>, actions: Flow<ExplorerAction>): Flow<ExplorerAction> {
        return merge(
            onOpenFileClicked(actions),
        )
    }

    private fun onOpenFileClicked(actions: Flow<ExplorerAction>): Flow<ExplorerAction> {
        return actions.filterIsInstance<ExplorerAction.UiAction.OnOpenFileClicked>()
            .transform { action ->
                editorInteractor.openFile(action.fileNode.file)
            }
    }
}