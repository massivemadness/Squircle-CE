...
class SearchNodeStrategyTest {

    @Test
    fun `When building node list with matching query Then append node and recurse if expanded`() {
        // Given
        ...
        val strategy = SearchNodeStrategy(options)
        ...
        val untitledNode = createFileNode(name = "untitled.txt")

        val nodeMap = hashMapOf(
            rootKey to listOf(folderNode),
            folderNode.key to listOf(fileNode, untitledNode),
        )
    ...
    @Test
    fun `When building node list with no match Then don't append or recurse`() {
        // Given
        val options = NodeBuilderOptions(
            searchQuery = "123",
            showHidden = false,
        ...
        // Then
        assertEquals(emptyList<FileNode>(), appended)
