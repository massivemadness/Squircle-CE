...
class FragmentDirectory : BaseFragment(), RecyclerSelection {
    ...
    override fun onClick(fileModel: FileModel) {
        if(fileModel.isFolder) {
            viewModel.fileTabsAddEvent.value = fileModel
        } else {
    ...
    override fun onLongClick(fileModel: FileModel): Boolean {
        return showChooseDialog(fileModel)
    }
            ...
            try { //Открытие файла через соответствующую программу
                context?.let {
                    ...
                    if(file.exists()) {
                        ...
                        val uri = FileProvider.getUriForFile(it, "$packageName.provider", file)
    ...
    private fun showChooseDialog(fileModel: FileModel): Boolean {
